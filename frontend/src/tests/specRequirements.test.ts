...
export function runAllSpecTests(): TestResult[] {
  ...
  executeTest(
    "TEST-RF01-02",
    "RF01 / RF02",
    "Create Visit and generate structured Sinclair report with codes",
    () => {
      ...
      assert(
        result.reporte.codigo_motivo === CodigoMotivo.SRT,
        "Motivo code should match",
      );
  ...
  executeTest(
    "TEST-RF04",
    "RF04 / RF09 / RF19",
    "Record replacement of Cabezal and update locations & statuses",
    () => {
      ...
      assert(
        retired.estado === Status.PENDING,
        "Retired machine status must be PENDING",
      );
      assert(
        retired.ubicacion === "EMP-04",
        "Retired machine location must be Workshop EMP-04",
      );
      assert(
        installed.estado === Status.USING,
        "Installed machine status must be USING",
      );
      assert(
        installed.ubicacion === "EMP-01",
        "Installed machine location must be EMP-01",
      );
  ...
  executeTest(
    "TEST-RF05-07",
    "RF05 / RF07 / RF18",
    "Record machine service, verify stock deduction and insufficient stock check",
    () => {
      ...
      const result = srvService.createService({
        ...
        consumibles: [
          { consumible_id: csm.id, nombre: csm.nombre, cantidad: 2 },
        ],
        tecnico_id: "TEC-01",
      });
      ...
      try {
        srvService.createService({
          ...
          consumibles: [
            { consumible_id: csm.id, nombre: csm.nombre, cantidad: 10 },
          ],
        });
      } catch (e) {
      ...
      assert(
        threw,
        "Should throw InsufficientStockError when requested stock exceeds available",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF08-20",
    "RF08 / RF20",
    "Record Cambio de Freno on-site, verify brake status and cabezal assignment",
    () => {
      ...
      assert(
        cabezal.freno_actual_id === "FRN-302",
        "Cabezal freno_actual_id must update to installed brake",
      );
      assert(
        frenoRet.estado === Status.PENDING,
        "Retired brake must be PENDING",
      );
      assert(
        frenoInst.estado === Status.USING,
        "Installed brake must be USING",
      );
      assert(
        frenoInst.cabezal_id === "CAB-101",
        "Installed brake cabezal_id must point to cabezal",
      );

      const cabCambios = cambioService.getCabezalCambios("CAB-101");
      assert(
        cabCambios.length > 0,
        "Cabezal change history must record the swap",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF13-14",
    "RF13 / RF14",
    "Toggle critical consumable status and trigger low stock alerts",
    () => {
      ...
      assert(
        updated.es_critico === true,
        "Consumable should be marked critical",
      );
      ...
      assert(
        Boolean(foundAlert),
        "Low stock alert should trigger for item under minimum",
      );
      assert(
        foundAlert?.es_critico === true,
        "Alert should flag critical state",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF13-MIN",
    "RF13",
    "Update Minimo Requerido (stock_minimo) and verify alert deficit update",
    () => {
      const service = new ConsumibleService();
      const item = service.create({
      ...
      const updated = service.getById(item.id);
      assert(
        updated.stock_minimo === 30,
        "stock_minimo should be updated to 30",
      );
      ...
      assert(
        Boolean(found),
        "Alert must trigger after updating minimum above current stock",
      );
  ...
  executeTest(
    "TEST-RF15",
    "RF15",
    "Identify packing plants without visits for more than 15 days",
    () => {
      const service = new EmpaqueService();
      const alerts = service.getUnvisitedAlerts();
      assert(
        alerts.length > 0,
        "Seed should contain plants not visited for >15 days (EMP-02, EMP-03)",
      );
      assert(
        alerts[0].dias_sin_visita >= 15,
        "Alert must have dias_sin_visita >= 15",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF10-EQUIPMENT",
    "RF10 / RF11",
    "Create Empaque with initial Cabezales (with attached Freno) and Caseteras assigned to plant",
    () => {
      ...
      assert(
        createdCab.ubicacion === empId,
        "Cabezal location must be set to new Empaque ID",
      );
      assert(
        createdCab.freno_actual_id === frenoId,
        "Cabezal must link to created Freno",
      );
      ...
      assert(
        createdFreno.ubicacion === empId,
        "Freno location must be set to new Empaque ID",
      );
      assert(
        createdFreno.cabezal_id === cabId,
        "Freno must link to created Cabezal",
      );
      ...
      assert(
        createdCas.ubicacion === empId,
        "Casetera location must be set to new Empaque ID",
      );

      // Verify location aggregated query
      const plantEquipment = machineService.getEquipmentByLocation(empId);
      assert(
        plantEquipment.cabezales.length === 1,
        "Plant must have 1 cabezal",
      );
      assert(plantEquipment.frenos.length === 1, "Plant must have 1 freno");
      assert(
        plantEquipment.caseteras.length === 1,
        "Plant must have 1 casetera",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF10-17",
    "RF10 / RF17",
    "Create technician, toggle status and compute service hours and visits",
    () => {
      ...
      assert(
        updated.estado === StatusTecnico.VACACIONES,
        "Status should be VACACIONES",
      );

      const stats = service.getActivityStats();
      assert(stats.length > 0, "Activity stats list must be returned");
      assert(
        typeof stats[0].total_visitas === "number",
        "Total visits must be number",
      );
      assert(
        typeof stats[0].total_horas_servicio === "number",
        "Total hours must be number",
      );
    },
  );
  ...
  executeTest(
    "TEST-RF21-23",
    "RF21 / RF22 / RF23",
    "Process batch of offline pending visits asynchronously",
    () => {
      ...
      assert(
        ...
        "Offline visit should be synced successfully",
      );
  ...
  executeTest(
    "TEST-RNF11",
    "RNF11",
    "Export telemetry payload for Sinclair external provider connector",
    () => {
      const providerService = new ExternalProviderService();
      const payload = providerService.exportProviderTelemetry();
      assert(
        payload.provider_name.includes("Sinclair"),
        "Provider name must be present",
      );
      assert(
        payload.total_machines_in_field > 0,
        "Total machines in field must be counted",
      );
      assert(
        Array.isArray(payload.synced_reports),
        "Reports data must be array",
      );
    },
  );
  ...
  executeTest(
    "TEST-AUDIT-TRAIL",
    "Audit Trail",
    "Maintain chronological audit log of changes across machines, consumables, and visits categorized by user ID and timestamp",
    () => {
      ...
      const logForCreation = db.auditLogs.find(
        (l) => l.targetId === testConsumible.id && l.action === "CREATE",
      );
      assert(
        Boolean(logForCreation),
        "Audit log entry must be created on consumable creation",
      );
      assert(
        logForCreation?.category === "consumable",
        "Audit entry category must be consumable",
      );
      assert(Boolean(logForCreation?.userId), "Audit entry must have userId");
      assert(
        Boolean(logForCreation?.timestamp),
        "Audit entry must have timestamp",
      );

      // 2. Log a consumable restock
      consumibleService.restock(testConsumible.id, 25);
      const logForRestock = db.auditLogs.find(
        (l) => l.targetId === testConsumible.id && l.action === "RESTOCK",
      );
      assert(
        Boolean(logForRestock),
        "Audit log entry must be recorded on restock",
      );
      assert(
        logForRestock?.details.includes("25 unidades"),
        "Restock details must capture added amount",
      );
      ...
      const logForVisit = db.auditLogs.find(
        (l) => l.targetId === visita.id && l.category === "visit",
      );
      assert(
        Boolean(logForVisit),
        "Audit log entry must be recorded on visit creation",
      );
      assert(
        logForVisit?.userId === "TEC-02",
        "Audit log must categorize event by user ID",
      );
    },
  );
  ...
  executeTest(
    "TEST-AUDIT-RBAC",
    "Audit Trail RBAC",
    "Restrict Audit Trail viewing and access control exclusively to administrators",
    () => {
      ...
      assert(
        ...
        "Admin user must be authorized for Audit Trail",
      );
      assert(
        isTechAuthorized === false,
        "Standard technician user must NOT be authorized for Audit Trail",
      );
    },
  );
